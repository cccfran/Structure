...
for(gr in 1:298) {
  ...
  iname <- (unlist(sapply(tree, function(x) names(x)[x==0])))
  i = (unlist(sapply(1:length(tree), function(x) rep(x, length(tree[[x]])-1))))
  j = (unlist(sapply(tree, function(x) names(x)[x!=0] )))
  x = (unlist(sapply(tree, function(x) x[x!=0] )))

  m <- rbind(m, sparseMatrix(i, match(j, nodes_name), x = x, 
                    dims = c(length(tree), length(nodes_name)),
                    dimnames = list(iname, nodes_name)))
}
...
saveRDS(m, paste0("../output_hpcc/tree_mat_", opt$year, ".RDS"))
